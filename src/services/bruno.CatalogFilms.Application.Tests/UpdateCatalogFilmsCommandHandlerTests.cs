...
{
    public class UpdateCatalogFilmsCommandHandlerTests
    {
        ...
        public UpdateCatalogFilmsCommandHandlerTests()
        {
        ...
        [Fact]
        public async Task ShouldUpdateCatalogFilmsWithSuccess()
        {
            var catalogFilms = new Domain.CatalogFilms().Create(_name, _description, _rating, _producer, _image, _year, _userId);

            Moq.Mock<IUnitOfWork> mock = new Moq.Mock<IUnitOfWork>();
            mock.Setup(x => x.Commit()).ReturnsAsync(true);

            _catalogFilmsRepository.GetById(catalogFilms.Id).Returns(catalogFilms);
            _catalogFilmsRepository.UnitOfWork = mock.Object;

            var command = new UpdateCatalogFilmsCommand(_id, _name, _description, _rating, _producer, _image, _year, _userId);

            var handler = new UpdateCatalogFilmsCommandHandler(_catalogFilmsRepository);
